<script lang="ts">
  ...
  function getSuspicionLevel(score: number): { label: string; color: string } {
    if (score >= 70) return { label: 'High', color: 'text-red-400 bg-red-900/40' };
    if (score >= 40) return { label: 'Medium', color: 'text-orange-400 bg-orange-900/40' };
    if (score >= 20) return { label: 'Low', color: 'text-yellow-400 bg-yellow-900/40' };
    return { label: 'Unlikely', color: 'text-green-400 bg-green-900/40' };
  }
</script>

<div class="space-y-4">
  <div class="flex items-center justify-between">
    <h2 class="text-lg font-semibold text-gray-100">Suspect Foods</h2>
          ...
          <div class="flex items-start justify-between">
            <div class="flex-1">
              <div class="flex items-center gap-2">
                <span class="font-medium text-gray-100 capitalize">{item.food}</span>
              ...
              {#if item.relatedSymptoms.length > 0}
                <div class="flex flex-wrap gap-1 mt-2">
                  {#each item.relatedSymptoms.slice(0, 3) as symptom}
                    <span class="text-xs bg-[#2a2a2a] text-gray-400 px-2 py-0.5 rounded">
            ...
            <div class="text-right">
              <div class="text-2xl font-bold {item.suspicionScore >= 50 ? 'text-red-400' : 'text-gray-500'}">
                {Math.round(item.suspicionScore)}%
              </div>
              <div class="text-xs text-gray-500">suspicion</div>
            </div>
          </div>
        </div>
      {/each}
